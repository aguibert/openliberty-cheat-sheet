...
Applications can pass messages among themselves with the Java Message Service (JMS) APIs. Liberty can act as a server of JMS destinations (queues and topics,) or it can connect to another server to access those destinations--either another Liberty server or IBM MQ.

=== JMS Server
Liberty can act as a JMS server whether or not it is serving any applications. 
...
<messagingEngine>
    <queue id="libertyQueue" />
    <topicSpace id="libertyTopic" />
</messagingEngine>
----
...
=== JMS Client
Liberty can allow applications to connect to both local and remote JMS destinations. (Local connections will also need the JMS server config from the previous section.)
...
Liberty can also provide administered objects for queues and topics that applications can use.

.server.xml
[source,xml]
----
<jmsTopic jndiName="jms/jmsLocalTopic">
    <properties.wasJms topicName="myTopic" topicSpace="libertyTopic" />
</jmsQueue>

<jmsQueue jndiName="jms/jmsRemoteQueue">
    <properties.wasJms queueName="libertyQueue" />
</jmsQueue>
----

=== IBM MQ Client
In addition to other Liberty servers, Liberty can connect to IBM MQ servers and allow applications to access destinations hosted in MQ Queue Managers. You don't need the `wasJmsClient` feature to connect to MQ.
...
<jmsQueueConnectionFactory id="MQRemoteQCF" jndiName="jms/mqRemoteQCF">
...
<jmsTopicConnectionFactory id="MQRemoteTCF" jndiName="jms/mqRemoteTCF">
